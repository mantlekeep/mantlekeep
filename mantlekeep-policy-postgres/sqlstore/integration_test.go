...
func TestStoreAgainstRealPostgres(t *testing.T) {
	...
	t.Run("an empty schema is an error, not an empty policy", func(t *testing.T) { anEmptySchemaIsAnError(t, db) })

	t.Run("a real database serves the revision the file source serves", func(t *testing.T) { aDatabaseServesTheFileSourcesRevision(t, db) })

	t.Run("seeding never overwrites a policy that already exists", func(t *testing.T) { seedingNeverOverwrites(t, db) })

	t.Run("a change that cannot be rendered writes nothing", func(t *testing.T) { anUnrenderableChangeWritesNothing(t, db) })

	t.Run("concurrent operators do not lose each other's changes", func(t *testing.T) { concurrentOperatorsDoNotLoseChanges(t, db) })

	t.Run("the stored documents migrate back to a file deployment", func(t *testing.T) { storedDocumentsMigrateBackToFiles(t, db) })

	t.Run("history answers what the policy said at a point in time", func(t *testing.T) { historyAnswersPointInTime(t, db) })
}

// an empty schema is an error, not an empty policy
func anEmptySchemaIsAnError(t *testing.T, db *sql.DB) {
...
// a real database serves the revision the file source serves
func aDatabaseServesTheFileSourcesRevision(t *testing.T, db *sql.DB) {
...
// seeding never overwrites a policy that already exists
func seedingNeverOverwrites(t *testing.T, db *sql.DB) {
...
// a change that cannot be rendered writes nothing
func anUnrenderableChangeWritesNothing(t *testing.T, db *sql.DB) {
...
// concurrent operators do not lose each other's changes
func concurrentOperatorsDoNotLoseChanges(t *testing.T, db *sql.DB) {
...
// the stored documents migrate back to a file deployment
func storedDocumentsMigrateBackToFiles(t *testing.T, db *sql.DB) {
...
// history answers what the policy said at a point in time
func historyAnswersPointInTime(t *testing.T, db *sql.DB) {
	store := seededStore(t, db)
	policy := pgpolicy.New(store)
